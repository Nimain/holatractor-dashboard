...
import { X, Search, Radio, Loader2, Layers } from "lucide-react";
...
interface AddAttachmentModalProps {
  isOpen: boolean;
  onClose: () => void;
  onAddSuccess?: (attachmentData: any) => void;
}
...
export default function AddAttachmentModal({
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (!attachmentType) {
      errorMessage("Please select an attachment type");
      return;
    }
    ...
    setTimeout(() => {
      setSubmitting(false);
      successMessage("Attachment added to fleet successfully!");
      if (onAddSuccess) {
        onAddSuccess({
          attachmentType,
          serialNumber,
    ...
    <div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/60 backdrop-blur-sm p-4 animate-in fade-in duration-200">
      <div className="bg-white rounded-2xl border border-slate-200 shadow-2xl w-full max-w-lg overflow-hidden flex flex-col max-h-[90vh]">
          ...
          <div className="flex items-center gap-2.5">
            ...
            <div>
              <h3 className="text-lg font-bold text-slate-900">
                {t("addNewAttachmentTitle")}
              </h3>
              <p className="text-xs text-slate-500">
                Register agricultural attachments and equipment details
              </p>
            </div>
          </div>
          <button
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-5 overflow-y-auto">
          {/* Attachment Type Searchable Select */}
          <div className="space-y-1.5">
            ...
            <div className="relative">
              ...
              >
                <option value="">
                  {t("searchAttachmentPlaceholder")}
                </option>
                <option value="Heavy Duty Plow 4X">Heavy Duty Plow 4X</option>
                <option value="Rotary Tiller 2000">Rotary Tiller 2000</option>
                <option value="Disc Harrow 300">Disc Harrow 300</option>
                <option value="Seed Planter Pro">Seed Planter Pro</option>
                <option value="Crop Sprayer Attachment">Crop Sprayer Attachment</option>
                <option value="Balers & Harvester Implement">Balers & Harvester Implement</option>
              </select>
            </div>
          </div>

          {/* Serial Number & Acquisition Date Grid */}
          ...
          <div className="flex justify-end gap-3 pt-4 border-t border-slate-100">
            ...
            <Button
              type="submit"
              disabled={submitting}
              className="bg-[#790000] hover:bg-[#570000] text-white font-extrabold text-xs px-6 py-2.5 rounded-xl shadow-md uppercase tracking-wider flex items-center gap-2"
            >
              {submitting ? (
                <>
