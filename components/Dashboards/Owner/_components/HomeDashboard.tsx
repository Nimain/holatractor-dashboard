...
import { Check, TractorIcon, Smartphone, Wrench, User, MapPin } from "lucide-react"
    ...
    <div className="mt-6">
      ...
      <div className="grid grid-cols-1 1050px:grid-cols-2 gap-6">
        ...
        <div className="relative rounded-[20px] h-80 1600px:h-96 overflow-hidden">
          ...
          {stores.length === 0 ? (
            <Card className="w-full shadow-lg h-full rounded-2xl">
                    ...
                    <div className="relative flex items-center justify-center w-full h-full">
                      ...
                      <div className="absolute bottom-4 left-4 right-4 z-20 flex items-center justify-between">
                        ...
                        <Link href={`/owner/stores/${details.id}`} className="mx-auto 768px:mx-0">
                          <Button className="inline-flex items-center px-6 py-3 text-white font-bold bg-black rounded-full shadow-lg border-2 border-transparent transition-all duration-300 transform hover:scale-105 group pointer-events-auto">
        ...
        <div className="p-0 rounded-[20px] shadow-lg h-80 1600px:h-96 w-full">
          ...
          {location.latitude && location.longitude ? (
            ...
            <div className="h-full flex items-center justify-center">
              <p>
                <TranslatedText greetings={OwnerDashboardTranslation.enableLocationDetails} />
              </p>
            </div>
          )}
        </div>
            ...
            <CardContent>
              <div className="relative my-2 mb-0 bottom-7">
                ...
                <Button
                  variant="default"
                  size="sm"
                  className="flex absolute top-1/2 left-14 transform -translate-x-1/2 -translate-y-1/2 rounded-full"
                >
                  <span>
                    <TranslatedText greetings={OwnerDashboardTranslation.seeProfile} />
                  </span>
                </Button>
              </div>
              <div className="mt-10 mb-6 space-y-4">
                <div className="flex justify-between items-center">
                  <span className="text-sm font-medium">
                    <TranslatedText greetings={operatorWorkPageTranslations.costPerJob} />:
                  </span>
                  <span className="text-sm font-bold">${operators[slideIndex].cost_per_job ?? "N/A"}</span>
                </div>
                <div className="flex justify-between items-center">
                  <span className="text-sm font-medium">
                    <TranslatedText greetings={operatorWorkPageTranslations.costPerHour} />:
                  </span>
                  <span className="text-sm font-bold">${operators[slideIndex].cost_per_hour ?? "N/A"}/hr</span>
                </div>
                <div className="flex justify-between items-center">
                  <span className="text-sm font-medium">
                    <TranslatedText greetings={operatorWorkPageTranslations.costPerMonth} />:
                  </span>
        ...
        <Card className="w-full shadow-lg rounded-2xl">
          <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
            ...
            <span className="text-sm text-muted-foreground">
              {tractorsInUse} <TranslatedText greetings={OwnerDashboardTranslation.inUse} />
            </span>
          </CardHeader>
          <CardContent>
            <p className="text-sm text-muted-foreground">
              <TranslatedText greetings={OwnerDashboardTranslation.tractorOperations} />
            </p>
            <div className="mt-4 text-3xl font-bold">
              {tractors.length}{" "}
              <span className="text-xl font-normal">
                <TranslatedText greetings={OwnerDashboardTranslation.totalTractors} />
              </span>
            </div>
            <div className="mt-4 text-sm text-muted-foreground flex justify-between">
              <span>0</span>
              <span>
                {tractorsInUse} <TranslatedText greetings={OwnerDashboardTranslation.tractorsInUse} />
              </span>
            </div>
          ...
          <CardContent>
            <div className="flex justify-between items-center">
              ...
              <span className="text-sm text-muted-foreground">
                {attachmentsInUse} <TranslatedText greetings={OwnerDashboardTranslation.inUse} />
              </span>
            </div>
            <p className="text-sm text-muted-foreground mt-1">
              <TranslatedText greetings={OwnerDashboardTranslation.variousImplements} />
            </p>
            <div className="mt-4 text-2xl font-bold">
              {attachments.length}{" "}
              <span className="text-lg font-normal">
                <TranslatedText greetings={OwnerDashboardTranslation.totalAttachments} />
              </span>
            </div>
            <div className="mt-4 text-sm text-muted-foreground flex justify-between">
              <span>0</span>
              <span>
                {attachmentsInUse} <TranslatedText greetings={OwnerDashboardTranslation.attachmentsInUse} />
              </span>
            </div>
            <div className="mt-2 flex items-center gap-x-1">
