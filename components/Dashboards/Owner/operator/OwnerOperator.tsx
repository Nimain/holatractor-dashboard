...
const OwnerOperator = () => {
    ...
    function fetchOperators() {
        ...
        renderInstance.get(`/owner/get-operators/${user.userId}?searchBy=${searchBy}&search=${searchTerm}&sortBy=${sortBy}&sortOrder=${sortOrder}&page=${page}`)
            .then((res) => {
                    ...
                    <TableBody>
                        {fetchingOperatorDetails ? (
                            ...
                            allOperators.map((customer, i) => {
                                const name = `${customer.operator.user.first_name} ${customer.operator.user.middle_name ?? ""} ${customer.operator.user.last_name}`
                                const email = customer.operator.user.email
                                return (
                                    <Dialog key={i}>
                                        <DialogTrigger asChild>
                                            <TableRow className="border-t" onMouseEnter={()=>{setHoveredItem(i)}} onMouseLeave={()=>{setHoveredItem(-1)}}>
                                                    ...
                                                    <div className="flex items-center gap-3">
                                                        ...
                                                        <div>
                                                            <p className="font-medium">
                                                                {hoveredItem === i ? name : name.slice(0, 4) + "..."}
                                                            </p>
                                                        </div>
                                                    </div>
                                                </TableCell>
                                                <TableCell className="p-4">
                                                    <p className="text-sm">{hoveredItem === i ? email : email.slice(0, 4) + "..."}</p>
                                                </TableCell>
                                                <TableCell className="p-4">
                                  ...
                                  `}>
                                                    ...
                                                    <span>
                                                        {hoveredItem === i ? customer.store.name : customer.store.name.slice(0, 4) + "..."}
                                                    </span>
                                                </TableCell>
                                                <TableCell className="p-4">
