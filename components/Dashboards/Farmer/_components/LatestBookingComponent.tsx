...
const LatestBookingComponent = ({booking, bookingLength}:{booking: Booking[]; bookingLength: number;}) => {
    ...
    if(booking.length === 0){
        return (
            <Card className="w-full max-w-sm bg-zinc-900 text-white border-none rounded-3xl">
    ...
    return (
      <Card className="w-full 900px:max-w-md bg-zinc-900 text-white border-none rounded-3xl">
          ...
          {booking.map((task) => (
            ...
            >
              <div className='flex items-center gap-3'>
                <div className='w-10 h-10 flex items-center justify-center aspect-square rounded-full bg-white/20'>
                    <Tractor className='text-white/70' />
                </div>
