...
function DealerDetailModal({
  ...
  useEffect(() => {
    if (dealer) {
      setFirstName(dealer.user.first_name || "");
      setMiddleName(dealer.user.middle_name || "");
      setLastName(dealer.user.last_name || "");
      setEmail(dealer.user.email || "");
      setMobile(dealer.user.mobile || "");
      setGender(dealer.user.gender || "male");
      setStatus(Number(dealer.status ?? dealer.Status ?? 1));
      setIsEditing(false);
    }
  }, [dealer]);
  ...
  const fullName = [dealer.user.first_name, dealer.user.middle_name, dealer.user.last_name]
    .filter(Boolean)
    .join(" ") || "Dealer";
  ...
  const handleSave = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const res = await axios.patch(
        ...
        {
          id: dealer.id,
          user_id: dealer.user_id || dealer.id,
      ...
      if (res.data?.success) {
        ...
        errorMessage(res.data?.message || "Failed to update dealer");
      }
    } catch (err: any) {
      errorMessage(err?.response?.data?.message || "Failed to update dealer");
    } finally {
      setSaving(false);
    }
  };
        ...
        <div className="bg-gradient-to-r from-blue-950 via-slate-900 to-indigo-950 p-6 text-white relative">
          ...
          <div className="flex items-center gap-4">
            <div className="relative w-14 h-14 rounded-2xl overflow-hidden bg-white/10 flex items-center justify-center flex-shrink-0 border border-white/20">
              {isValidImageUrl(dealer.user.image) ? (
                <Image
                ...
                <span className="text-xl font-bold text-white">
                  {(dealer.user.first_name?.[0] || "D").toUpperCase()}
                </span>
              )}
            </div>
            <div className="min-w-0">
              <span className="inline-flex items-center gap-1.5 px-2.5 py-0.5 rounded-full text-xs font-semibold bg-blue-500/20 text-blue-300 border border-blue-400/30 mb-1">
                <Store size={12} />
                Commercial Dealer
              </span>
              <h3 className="text-lg font-bold text-white truncate">{fullName}</h3>
              <p className="text-xs text-slate-300 truncate">ID: {dealer.id}</p>
            </div>
          </div>
        </div>
          ...
          {!isEditing ? (
            <div className="space-y-4">
              <div className="grid grid-cols-2 gap-4">
                ...
                <div className="p-3 bg-gray-50 rounded-xl border border-gray-100">
                  ...
                  <p className="text-sm font-semibold text-gray-800 mt-0.5">
                    {dealer.user.mobile
                      ? `${dealer.user.country_code || "+591"} ${dealer.user.mobile}`
                      : "No phone"}
                  </p>
                </div>
                <div className="p-3 bg-gray-50 rounded-xl border border-gray-100">
                  ...
                  <p className="text-sm font-semibold text-gray-800 mt-0.5 capitalize">
                    {dealer.user.gender || "Not specified"}
                  </p>
                </div>
                <div className="p-3 bg-gray-50 rounded-xl border border-gray-100">
                    ...
                    >
                      <span
                        className={`w-1.5 h-1.5 rounded-full ${
                          (dealer.status ?? dealer.Status) === 1 ? "bg-emerald-500 animate-pulse" : "bg-red-400"
                        }`}
                      />
                      {(dealer.status ?? dealer.Status) === 1 ? "Active" : "Inactive"}
                    </span>
                  </p>
                </div>
                <div className="p-3 bg-gray-50 rounded-xl border border-gray-100">
              ...
              <div className="p-3 bg-gray-50 rounded-xl border border-gray-100 space-y-1 text-xs text-gray-500">
                <p>
                  <span className="font-semibold text-gray-700">Created:</span> {fmtDate(dealer.createdAt)}
                </p>
                <p>
                  <span className="font-semibold text-gray-700">Updated:</span> {fmtDate(dealer.updatedAt)}
                </p>
              </div>
              ...
              <div className="flex justify-end gap-3 pt-2">
                ...
                <Button
                  ...
                  className="bg-blue-600 hover:bg-blue-700 text-white rounded-xl px-4 text-xs font-semibold flex items-center gap-1.5 shadow-sm"
                >
                  <Edit size={13} />
                  Edit Dealer
                </Button>
            ...
            <form onSubmit={handleSave} className="space-y-4">
              <div className="grid grid-cols-2 gap-3">
                <div>
                  <Label className="text-xs font-semibold text-gray-700 mb-1 block">First Name / Company</Label>
                  <Input
                    value={firstName}
              ...
              <div className="grid grid-cols-2 gap-3">
                ...
                <div>
                  <Label className="text-xs font-semibold text-gray-700 mb-1 block">Dealership Status</Label>
              ...
              <div className="flex justify-end gap-3 pt-3 border-t border-gray-100">
                ...
                <Button
                  type="submit"
                  disabled={saving}
                  className="bg-blue-600 hover:bg-blue-700 text-white rounded-xl px-5 text-xs font-semibold shadow-sm"
                >
...
/* ── Main Dealer Section Component ────────────────────────────── */
const DealerSection = () => {
  const [loading, setLoading] = useState(true);
  const [refreshing, setRefreshing] = useState(false);
  const [allDealers, setAllDealers] = useState<DealerItem[]>([]);
  ...
  // ── Fetch Dealers from FastAPI / Next.js API Route ─────────
  const fetchAllDealers = useCallback(async (isRefresh = false) => {
    if (isRefresh) setRefreshing(true);
    else setLoading(true);

    try {
      const res = await axios.get("/api/dealer", { timeout: 8000 });
      let list: DealerItem[] = [];

      if (Array.isArray(res.data)) {
        list = res.data;
      } else if (res.data?.dealers && Array.isArray(res.data.dealers)) {
        list = res.data.dealers;
      }

      setAllDealers(list);
    } catch (err) {
      console.error("Error fetching dealer list:", err);
      errorMessage("Failed to load dealers from FastAPI");
    } finally {
      setLoading(false);
      setRefreshing(false);
    }
  ...
  const stats = useMemo(() => {
    ...
    const active = allDealers.filter((d) => (d.status ?? d.Status) === 1).length;
    const inactive = allDealers.filter((d) => (d.status ?? d.Status) === 0).length;
    const verified = allDealers.filter((d) => d.user?.emailVerified).length;
  ...
  const filteredDealers = useMemo(() => {
    ...
    if (statusFilter === "active") {
      list = list.filter((d) => (d.status ?? d.Status) === 1);
    } else if (statusFilter === "inactive") {
      list = list.filter((d) => (d.status ?? d.Status) === 0);
    }
    ...
    if (searchQuery.trim()) {
      ...
      list = list.filter((d) => {
        const fullName = `${d.user?.first_name || ""} ${d.user?.middle_name || ""} ${d.user?.last_name || ""}`.toLowerCase();
        const email = (d.user?.email || "").toLowerCase();
        const mobile = (d.user?.mobile || "").toLowerCase();
    ...
    return list;
  }, [allDealers, statusFilter, searchQuery, sortBy, sortAsc]);

  // Pagination calculation
  const totalPages = Math.max(1, Math.ceil(filteredDealers.length / itemsPerPage));
  const paginatedDealers = useMemo(() => {
    const start = (currentPage - 1) * itemsPerPage;
    return filteredDealers.slice(start, start + itemsPerPage);
  ...
  return (
    <div className="w-full min-h-screen p-4 sm:p-6 lg:p-8 space-y-6">
      {/* ── Top Header Banner ─────────────────────────────────────── */}
      <div className="bg-gradient-to-r from-blue-950 via-slate-900 to-indigo-950 rounded-3xl p-6 sm:p-8 text-white shadow-xl relative overflow-hidden border border-blue-800/40">
        ...
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-6 relative z-10">
          <div>
            ...
            <h1 className="text-2xl sm:text-4xl font-extrabold tracking-tight">
              Dealers Directory & Franchises
            </h1>
            <p className="text-blue-100/70 text-sm mt-1 max-w-xl">
              Manage commercial machinery dealerships, equipment franchises, and verified partners across {stats.total} accounts with live FastAPI synchronization.
            </p>
          </div>
          ...
          <div className="flex items-center gap-2.5 flex-wrap">
            ...
            <Button
              ...
              className="bg-blue-600 hover:bg-blue-700 text-white text-xs font-bold rounded-xl h-10 px-4 shadow-lg shadow-blue-600/20 transition-all active:scale-[0.98]"
            >
        ...
        <div className="relative flex-1 min-w-[260px]">
          ...
          <Input
            ...
            placeholder={getTranslation(locale, {
              en: "Search dealers by name, dealership, email, phone, or ID...",
              es: "Buscar concesionarios por nombre, correo, teléfono o ID...",
              ay: "Alxiri thaqaña suti, chaski, celular...",
              qu: "Dealer maskay sutinwan, chaskinwan...",
              gn: "Heka dealer téra, ñanduti veve rupi...",
            })}
          ...
          <table className="w-full text-left text-sm border-collapse">
            <thead>
              <tr className="bg-gray-50/80 border-b border-gray-200/80 text-[11px] font-bold text-gray-500 uppercase tracking-wider">
                ...
                <th className="py-3.5 px-5">Dealership / Owner Name</th>
              ...
              {loading ? (
                <tr>
                  <td colSpan={7} className="py-16 text-center text-gray-400">
                    ...
                    <p className="text-sm font-semibold">Loading dealers from FastAPI…</p>
                  </td>
                </tr>
                ...
                paginatedDealers.map((item, idx) => {
                  ...
                  return (
                    ...
                    >
                      ...
                      {/* Dealership Name & Avatar */}
                      <td className="py-3.5 px-5">
                        <div className="flex items-center gap-3">
                          <div className="relative w-9 h-9 rounded-xl overflow-hidden bg-gradient-to-br from-blue-600 to-indigo-800 text-white flex items-center justify-center font-bold text-xs flex-shrink-0 shadow-sm">
                      ...
                      <td className="py-3.5 px-5 text-right">
                        <Button
                          ...
                          className="h-7 text-xs px-3 rounded-lg border-blue-200 text-blue-700 hover:bg-blue-50 font-semibold"
                        >
                          Edit / View
                        </Button>
                      </td>
      ...
      <div className="md:hidden space-y-3">
        {loading ? (
          <div className="py-16 text-center text-gray-400 bg-white rounded-2xl border border-gray-100 p-6">
            <RefreshCw className="h-8 w-8 text-blue-500 animate-spin mx-auto mb-2" />
            <p className="text-sm font-semibold">Loading dealers…</p>
          </div>
        ) : paginatedDealers.length === 0 ? (
          <div className="py-16 text-center text-gray-400 bg-white rounded-2xl border border-gray-100 p-6">
            <Store className="h-10 w-10 text-gray-300 mx-auto mb-2" />
            ...
            return (
              ...
              >
                <div className="flex items-start justify-between gap-3">
                  <div className="flex items-center gap-3">
                    <div className="relative w-10 h-10 rounded-xl overflow-hidden bg-gradient-to-br from-blue-600 to-indigo-800 text-white flex items-center justify-center font-bold text-sm flex-shrink-0">
                      {isValidImageUrl(item.user?.image) ? (
                        <Image
                          src={item.user?.image!}
                          alt={fullName}
                ...
                <div className="pt-2 border-t border-gray-100 flex items-center justify-between text-xs text-gray-500">
                  ...
                  <Button
                    ...
                    className="h-7 text-xs px-3 rounded-lg border-blue-200 text-blue-700 font-semibold"
                  >
                    Edit / View
                  </Button>
                </div>
        ...
        <div className="bg-white p-4 sm:p-5 rounded-2xl border border-gray-200/80 shadow-sm flex flex-col md:flex-row items-center justify-between gap-4">
          ...
          <div className="flex flex-wrap items-center gap-3 text-xs text-gray-500">
            <span>
              ...
              of <span className="font-bold text-gray-900">{filteredDealers.length}</span> dealers
            </span>
      ...
      <Dialog open={openRegister} onOpenChange={setOpenRegister}>
        <DialogContent className="bg-white p-0 rounded-2xl border border-gray-100 shadow-2xl max-w-[460px] overflow-hidden">
          <div className="bg-gradient-to-r from-blue-950 via-slate-900 to-indigo-950 p-6 text-white relative">
            <p className="text-xs uppercase tracking-wider font-semibold text-blue-300 mb-1">Dealer Network</p>
            <h2 className="text-xl font-bold">Register New Dealer</h2>
            <p className="text-xs text-slate-300 mt-1">Enter dealership or commercial legal entity name.</p>
          </div>

          <div className="p-6 space-y-4">
            <div>
              <Label className="block text-xs font-semibold text-slate-700 uppercase tracking-wider mb-2">
                Dealership / Legal Entity Name *
              </Label>
              <Input
                value={newDealerName}
                onChange={(e) => handleNameChange(e.target.value)}
                placeholder="e.g. AgroTech Machinery Solutions"
                className="w-full px-4 py-2.5 text-sm bg-slate-50 border border-slate-200 rounded-xl focus:bg-white focus:border-blue-500 focus:ring-4 focus:ring-blue-500/10 outline-none transition-all font-medium"
              />
            </div>
          </div>
          ...
          <div className="p-5 bg-slate-50 border-t border-slate-100 flex justify-end gap-3">
            ...
            {isSignUpCard ? (
              ...
              <Button
                onClick={() => errorMessage("Please provide first and last name for dealership")}
                className="px-5 py-2 bg-blue-600 hover:bg-blue-700 text-white text-xs font-semibold rounded-xl shadow-md transition-all active:scale-[0.98]"
              >
                Next Step →
              </Button>
            )}
