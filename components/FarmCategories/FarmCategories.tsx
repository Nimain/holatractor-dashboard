...
const FarmCategoriesSection = () => {
  ...
  const handleUpdate = async () => {
    ...
    try {
      ...
      // Always include fields that have been modified
      if (formData.name.trim() !== selectedCategory.name) {
        payload.name = formData.name.trim();
      }
      
      // Handle icon - compare trimmed values, consider null and empty string as equivalent
      ...
      // Handle description - compare trimmed values, consider null and empty string as equivalent
  ...
  const handleDelete = async (id: string) => {
    if (!confirm("Are you sure you want to delete this category? This will fail if any farm items are using this category.")) {
      return;
    }
    ...
    try {
      ...
      const response = await renderInstance.delete(
        `/farm/categories/${id}`,
        getAuthHeaders()
      );
            ...
            <div key={category.id} className="bg-white rounded-lg shadow-md p-4 hover:shadow-lg transition-shadow">
              ...
              <div className="flex gap-2">
                ...
                <button
                  ...
                  className="flex-1 px-3 py-2 bg-yellow-100 hover:bg-yellow-200 text-yellow-700 rounded-lg font-medium transition-colors flex items-center justify-center gap-2"
                >
                  <Edit2 size={16} />
                  Edit
                </button>
          ...
          <div className="bg-white rounded-lg max-w-md w-full p-6">
            ...
            <div className="space-y-2 text-sm bg-gray-50 p-4 rounded-lg mb-4">
              <div className="flex justify-between">
                ...
                <span>{selectedCategory._count.farmItems}</span>
              </div>
              <div className="flex justify-between">
                <span className="font-medium">
                  {getTranslation(locale, {
              ...
              <div className="flex justify-between">
                ...
                <span className="text-xs font-mono">{selectedCategory.id}</span>
              </div>
            </div>
