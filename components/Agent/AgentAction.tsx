...
const AgentAction = ({ index, name, mailHover, email, emailVerified, creatDate, updateDate, status, id }: AgentActionProps) => {
    ...
    function InactiveAgent(e: MouseEvent<HTMLButtonElement, globalThis.MouseEvent>) {
        ...
        e.preventDefault()
        setLoading(true)
        renderInstance.patch(`/agent/inactivate_agent/${id}`, {}, {
            headers: {
    ...
    function ActiveAgent(e: MouseEvent<HTMLButtonElement, globalThis.MouseEvent>) {
        ...
        e.preventDefault()
        setLoading(true)
        renderInstance.patch(`/agent/activate_agent/${id}`, {}, {
            headers: {
                Authorization: `Bearer ${access_token}`,
            },
        }).then(() => {
            ...
            <SheetContent>
                ...
                <SheetFooter>
                    <SheetClose asChild>
                        {
                            status === 1 ?
                                <Button variant={"destructive"} onClick={(e)=>{InactiveAgent(e)}}>
                                    {
                                        loading ? <CircularProgress /> : "Inactive"
                                    }
                                </Button>
                                :
                                <Button className="bg-green-800" onClick={(e)=>{ActiveAgent(e)}}>
                                    {
