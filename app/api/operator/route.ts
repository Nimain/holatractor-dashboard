...
export async function GET(request: NextRequest) {
  try {
    ...
    try {
      ...
      console.warn("[/api/operator] FastAPI error:", fastErr?.message);
    }
    ...
    try {
      ...
      try {
        ...
        const operatorsList = res.rows.map((m: any) => ({
          id: String(m.id),
          user_id: String(m.user_id),
          role_id: String(m.role_id || "operator_role"),
          ...
          user: {
            id: String(m.user_id),
            first_name: String(m.first_name || "Operator"),
        ...
        return NextResponse.json(operatorsList);
      } finally {
        client.release();
      }
    } catch (dbErr: any) {
      console.warn("[/api/operator] Database query fallback:", dbErr?.message);
    }

    // 3. Try online NestJS /operator
    try {
      const backendRes = await axios.get(`${NestJsBaseURL}operator`, {
        headers,
        timeout: 4000,
      });
    ...
    console.error("[/api/operator] Fatal error:", error);
    return NextResponse.json([]);
  }
}
