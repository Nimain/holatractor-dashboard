...
export async function POST(
  ...
  const body = await request.json().catch(() => ({ command: "STATUS#" }));

  for (const key of AUTH_KEYS) {
    try {
      const res = await axios.post(`${baseUrl}/api/device/${imei}/command`, body, {
        headers: {
      ...
      if (e.response?.status === 404) {
        return NextResponse.json(
          { error: `Device ${imei} is offline or not connected to TCP socket` },
          { status: 404 }
        );
      }
    }
  ...
  return NextResponse.json(
    { error: "Failed to dispatch command to device" },
    { status: 500 }
  );
}
