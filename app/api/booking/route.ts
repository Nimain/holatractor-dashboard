...
export async function GET(request: NextRequest) {
  try {
    ...
    const bookingType = searchParams.get("type"); // "store" | "standalone_tractor" | "standalone_attachment" | null

    // 1. Query Render PostgreSQL directly
    try {
      ...
      try {
        ...
        if (bookingType === "store") {
          ...
          `;
        } else if (bookingType === "standalone_attachment") {
          query += ` 
        ...
        // Fetch sub-items (Tractors, Attachments, Standalone) for the bookings
        const bookingIds = res.rows.map((r) => r.id);
        let standaloneTractorsMap: Record<string, any[]> = {};
        let standaloneAttachmentsMap: Record<string, any[]> = {};
        let bookingTractorsMap: Record<string, any[]> = {};
        let bookingAttachmentsMap: Record<string, any[]> = {};

        if (bookingIds.length > 0) {
          try {
            const stRes = await client.query(
              `
              SELECT bst.*, t.name, t.model, t.images, t.type
              FROM "BookingStandaloneTractor" bst
              LEFT JOIN "Tractor" t ON t.id = bst."tractorId"
              WHERE bst."bookingId" = ANY($1::text[])
            `,
              [bookingIds]
            );
            stRes.rows.forEach((r) => {
              if (!standaloneTractorsMap[r.bookingId])
                standaloneTractorsMap[r.bookingId] = [];
              standaloneTractorsMap[r.bookingId].push(r);
            });

            const saRes = await client.query(
              `
              SELECT bsa.*, a.name, a.description, a.images
              FROM "BookingStandaloneAttachment" bsa
              LEFT JOIN "Attachment" a ON a.id = bsa."attachmentId"
              WHERE bsa."bookingId" = ANY($1::text[])
            `,
              [bookingIds]
            );
            saRes.rows.forEach((r) => {
              if (!standaloneAttachmentsMap[r.bookingId])
                standaloneAttachmentsMap[r.bookingId] = [];
              standaloneAttachmentsMap[r.bookingId].push(r);
            });

            const btRes = await client.query(
              `
              SELECT bt.*, tis.lat, tis.lan as lng, tis.hourly_price, t.name, t.model, t.images
              FROM "BookingTractor" bt
              LEFT JOIN "TractorInStore" tis ON tis.id = bt."tractorId"
              LEFT JOIN "Tractor" t ON t.id = tis."baseTractorId"
              WHERE bt."bookingId" = ANY($1::text[])
            `,
              [bookingIds]
            );
            btRes.rows.forEach((r) => {
              if (!bookingTractorsMap[r.bookingId])
                bookingTractorsMap[r.bookingId] = [];
              bookingTractorsMap[r.bookingId].push(r);
            });

            const baRes = await client.query(
              `
              SELECT ba.*, ais.hourly_price, a.name, a.description, a.images
              FROM "BookingAttachment" ba
              LEFT JOIN "AttachmentInStore" ais ON ais.id = ba."attachmentId"
              LEFT JOIN "Attachment" a ON a.id = ais."baseAttachmentId"
              WHERE ba."bookingId" = ANY($1::text[])
            `,
              [bookingIds]
            );
            baRes.rows.forEach((r) => {
              if (!bookingAttachmentsMap[r.bookingId])
                bookingAttachmentsMap[r.bookingId] = [];
              bookingAttachmentsMap[r.bookingId].push(r);
            });
          } catch (subErr: any) {
            console.warn("[/api/booking] Sub-items query notice:", subErr?.message);
          }
        }

        const mappedBookings = res.rows.map((r) => {
          const st = standaloneTractorsMap[r.id] || [];
          const sa = standaloneAttachmentsMap[r.id] || [];
          const bt = bookingTractorsMap[r.id] || [];
          const ba = bookingAttachmentsMap[r.id] || [];
          ...
          return {
            ...
            BookingStandaloneTractor: st,
            BookingStandaloneAttachment: sa,
            BookingTractor: bt,
            BookingAttachment: ba,
          };
        });
      ...
      console.warn("[/api/booking] DB direct notice:", dbErr?.message);
    }
    ...
    console.error("[/api/booking] GET error:", error?.message);
